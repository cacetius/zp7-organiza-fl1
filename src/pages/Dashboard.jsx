...
export default function Dashboard() {
  ...
  const perdasBrutasTurno = lossesTurno.filter(l => l.motivo_perda !== "ganho" && l.item_perda && l.hora).reduce((s, l) => s + (l.carros_perdidos || 0), 0);

  // Ganhos do turno
  const ganhosTurno = lossesTurno.filter(l => l.motivo_perda === "ganho" && l.item_perda && l.hora).reduce((s, l) => s + (l.carros_perdidos || 0), 0);
      ...
      {/* Visão do turno atual */}
       ...
       <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          { label: `Perda Real ${shiftLabel}`, value: perdaRealTurno, icon: TrendingDown, color: perdaRealTurno > 0 ? "text-red-400" : "text-muted-foreground", bg: perdaRealTurno > 0 ? "bg-red-500/10" : "bg-muted/30", border: perdaRealTurno > 0 ? "border-red-500/20" : "border-border" },
